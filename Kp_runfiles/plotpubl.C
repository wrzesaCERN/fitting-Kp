...
void plotpubl()
{
 ...
 //     }
  ...
  for (int icent=0; icent<6; icent++) {
    ...
    cf3 = (TH1D *) gDirectory->Get(Form("datapfpmbgC%i", icent));
    cf4 = (TH1D *) gDirectory->Get(Form("datapfmpbgC%i", icent));
    ...
    if (cf1) {
      ...
      canfitC0->SaveAs(Form("./plots/canfitC%i.root",icent));
      canfitC0->SaveAs(Form("./plots/canfitC%i.eps",icent));
      canfitC0->SaveAs(Form("./plots/canfitC%i.png",icent));
    }

    cf1ss = (TH1D *) gDirectory->Get(Form("datamfppbgC%i", icent));
    cf2ss = (TH1D *) gDirectory->Get(Form("datamfmmbgC%i", icent));
    cf3ss = (TH1D *) gDirectory->Get(Form("datapfppbgC%i", icent));
    cf4ss = (TH1D *) gDirectory->Get(Form("datapfmmbgC%i", icent));
    ...
    if (cf1ss) {
      ...
      canfitC0ss->SaveAs(Form("./plots/canfitC%iss.root",icent));
      canfitC0ss->SaveAs(Form("./plots/canfitC%iss.eps",icent));
      canfitC0ss->SaveAs(Form("./plots/canfitC%iss.png",icent));
    }
  }
  ...
  canradfit->SaveAs("./plots/canradpubl.eps");
  canradfit->SaveAs("./plots/canradpubl.png");
  canradfit->SaveAs("./plots/canradpubl.root");
